/*
 * arch/mips/brcmstb/brcm97400a0/bchip.c
 *
...
#if 0
    // PR 29586: don't touch LLMB or RAC settings
    _RAC_init();
#endif
...
#define DEV_RD(x)	(*((volatile uint32_t *)(x)))
#define DEV_WR(x, y)	do { *((volatile uint32_t *)(x)) = (y); } while(0)
...
int rac_setting(int value)
{
	...
	sprintf(msg, "before init RAC 0x%08x    0x%08x    0x%08x\n", 
			DEV_RD(rac_config0), DEV_RD(rac_config1),
			DEV_RD(rac_address_range));
	uart_puts(msg);
	...
	switch(value) {
		...
		case 11:    /* I/D-RAC enabled, C_INV='1'b, PF_D='1'b, PF_I='1'b, RAC_D='1'b, RAC_I='1'b */
			...
			break;
		case 0xff:  /* leave existing RAC setting intact */
			bcm_inv_rac_all();
			rac_value = 0;
			break;
	...
	if(value != 0xff)
	{
		...
		DEV_WR(rac_config0, (DEV_RD(rac_config0) & ~(3 << 18)) | (1 << 6));
		...
		/* PR 29586: 7400b0 Linux should not initialize RAC */
		printk("******* RAC initialization is performed by CFE on this platform\n");
	}
