/*
 *  arch/mips/brcmstb/brcm97329/brcmpci.c -- Broadcom BCM97329 pci setup routines
 *
...
#if defined( CONFIG_MIPS_BCM7329 ) &&  defined( CONFIG_MIPS_BCM7329A0_IDE_WAR )
...
int brcm_pcibios_enable_device(struct pci_dev *dev)
{
	...
	/* There is only 1 device on the 7329 at slot 0 */
	if (0 != PCI_SLOT(dev->devfn)) {
		return 0;
	}
...
printk("SATA config 0x3c orig : %08x\n", regData);

		regData = 0x0f;
		pci_write_config_byte(dev, 0x3e, regData); /* Minimum Grant, to avoid Latency being reset to lower value */
		
		regData = 0xff;
		pci_write_config_byte(dev, 0x3f, regData); /* SATA_CFG_MASTER_LATENCY_TIMER. */

pci_read_config_dword(dev,0x3c, &regData);
printk("SATA config 0x3c after : %08x\n", regData);
...
BCM7329_PCI_WAR
		...
		// Turn off SATA 2
		temp = *((volatile unsigned long *)KSEG1ADDR(PCI_SATA_PHYS_MEM_WIN5_BASE+0x148));
		temp |= 0x00000001;
BCM7329_PCI_WAR
		*((volatile unsigned long *)KSEG1ADDR(PCI_SATA_PHYS_MEM_WIN5_BASE+0x148)) = temp;
BCM7329_PCI_WAR
		printk("SATA: turning off 2nd ch reg = 0x%08lx.\n", *((volatile unsigned long *)KSEG1ADDR(PCI_SATA_PHYS_MEM_WIN5_BASE+0x148)));
BCM7329_PCI_WAR

#if !defined( CONFIG_PRODUCTION_CODES ) && !defined(CONFIG_MIPS_BCM7329A0_IDE_WAR )
// No need to call BCM7329_PCI_WAR here, we are definitely in DMA mode for this.
		// Turn on SATA error checking, for development debugging only.  Turn this off in production codes.
		*((volatile unsigned long *)KSEG1ADDR(PCI_SATA_PHYS_MEM_WIN5_BASE+0x88)) |=  0xffffffff & ~(0x94050000);
		printk("SATA: SIMR = 0x%08lx.\n", *((volatile unsigned long *)KSEG1ADDR(PCI_SATA_PHYS_MEM_WIN5_BASE+0x88)));
		printk("SATA: SCR1 = 0x%08lx.\n", *((volatile unsigned long *)KSEG1ADDR(PCI_SATA_PHYS_MEM_WIN5_BASE+0x44)));
