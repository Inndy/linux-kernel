...
{
      return readb(KSEG1ADDR((unsigned long) info->iomem_base +
...
static _INLINE_ void serial_out(struct async_struct *info, int offset,
				int value)
{
	writeb(value, KSEG1ADDR((unsigned long) info->iomem_base +
		(offset<<info->iomem_reg_shift)));
...
*((volatile unsigned long *) 0xbfc00004) = irq;
...
/*	printk("starting up ttys%d (irq %d)...", info->line, state->irq);*/
	...
	if (state->irq && (!IRQ_ports[state->irq] ||
		...
		retval = request_irq(state->irq, handler, SA_INTERRUPT|SA_SHIRQ,
