...
// JHK 08.03.09
static int part_getcfiids(struct mtd_info *mtd, unsigned char ids)
{
	struct mtd_part *part = PART(mtd);
	return part->master->getcfiids(part->master, ids);
}
...
{
	struct mtd_part *slave;
//hmx.modify.hee.plat2.starts
	struct mtd_part *part2unlock;
	u_char readdata=0;
	size_t rdcnt=0;
//hmx.modify.hee.plat2.ends
	...
	for (i = 0; i < nbparts; i++) {
		...
		if (master->getcfiids)// JHK 08.03.09
			slave->mtd.getcfiids = part_getcfiids;		
...
//hmx.modify.hee.plat2.starts
	part_unlock((struct mtd_info *)part2unlock,0,part2unlock->mtd.size);
	part_read((struct mtd_info *)part2unlock,0,1,&rdcnt,&readdata);
//hmx.modify.hee.plat2.ends
	return 0;
}
