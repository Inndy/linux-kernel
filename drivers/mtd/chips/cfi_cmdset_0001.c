 ...
 *
 * $Id: cfi_cmdset_0001.c,v 1.189 2006/02/08 16:46:35 nico Exp $
 *
 *
...
static int cfi_intelext_getcfiids (struct mtd_info *mtd, unsigned char ids); // JHK 08.03.09
...
#ifdef CONFIG_FLASH_LOCK_CTRL
static int __xipram do_xxlock_oneblockHUMAX(struct map_info *map, struct flchip *chip,
...
read_pri_intelext(struct map_info *map, __u16 adr)
{
 ...
 again:
	...
	if (!extp)
		return NULL;
#if 1 //hee.plat2
		if(extp->MinorVersion == '5')
		{
			extp->MinorVersion = '4'; //P33 CFI version is 1.5, make it 1.4 forcly!
		}
#endif //hee
...
struct mtd_info *cfi_cmdset_0001(struct map_info *map, int primary)
{
	...
	mtd->getcfiids = cfi_intelext_getcfiids; // JHK 08.03.09
				     ...
				     unsigned long adr, map_word datum, int mode)
{
...
#ifdef CONFIG_FLASH_LOCK_CTRL
	do_xxlock_oneblockHUMAX(map, chip, adr, 0, DO_XXLOCK_ONEBLOCK_UNLOCK);
#endif
	spin_lock(chip->mutex);
 ...
 out:	put_chip(map, chip, adr);
	spin_unlock(chip->mutex);
#ifdef CONFIG_FLASH_LOCK_CTRL
	do_xxlock_oneblockHUMAX(map, chip, adr, 0, DO_XXLOCK_ONEBLOCK_LOCK);
#endif
	return ret;
}
				    ...
				    unsigned long *pvec_seek, int len)
{
	...
	*pvec_seek = vec_seek;
	...
	if (map_word_bitsset(map, status, CMD(0x1a))) {
		...
		if (chipstatus & 0x02) {
			printk(KERN_ERR "%s: buffer write error (Read Only File System : EROFS)\n", map->name);
 			ret = -EROFS;
		} else if (chipstatus & 0x08) {
				...
				unsigned long count, loff_t to, size_t *retlen)
{
	...
	*retlen = 0;
...
#ifdef CONFIG_FLASH_LOCK_CTRL
	lock_addr = to;
	do_xxlock_oneblockHUMAX(map, &cfi->chips[chipnum], lock_addr, 0, DO_XXLOCK_ONEBLOCK_UNLOCK);
#endif
	...
	do {
		...
		{
#ifdef CONFIG_FLASH_LOCK_CTRL
			do_xxlock_oneblockHUMAX(map, &cfi->chips[chipnum], lock_addr, 0, DO_XXLOCK_ONEBLOCK_LOCK);
#endif
			return ret;
		}
				      ...
				      unsigned long adr, int len, void *thunk)
{
 ...
 retry:
#ifdef CONFIG_FLASH_LOCK_CTRL
	do_xxlock_oneblockHUMAX(map, chip, adr, len, DO_XXLOCK_ONEBLOCK_UNLOCK);
#endif
	spin_lock(chip->mutex);
	...
	if (map_word_bitsset(map, status, CMD(0x3a))) {
		...
		if ((chipstatus & 0x30) == 0x30) {
			...
			printk(KERN_ERR "%s: protection bit set : (status 0x%lx)\n", map->name, chipstatus);
 			/* Protection bit set */
			ret = -EROFS;
...
static int cfi_intelext_getcfiids (struct mtd_info *mtd, unsigned char ids)// JHK 08.03.09
{
	...
	{
		case 0:
	printk(KERN_ERR "[cfi_intelext_getcfiids] manufatureId:0x%X\n",cfi->mfr);
			return cfi->mfr;
		case 1:
	printk(KERN_ERR "[cfi_intelext_getcfiids] deviceId:0x%X\n",cfi->id);
			return  cfi->id;
		default:
			break;
	}
	...
	for ( ; chip_num < cfi->numchips; chip_num += chip_step) {
		...
		while (len > 0) {
			...
			if (++field == extp->NumProtectionFields)
				break;
			reg_prot_offset = le32_to_cpu(otp->ProtRegAddr);
			reg_fact_groups = le16_to_cpu(otp->FactGroups);
			reg_fact_size = 1 << otp->FactProtRegSize;
			reg_user_groups = le16_to_cpu(otp->UserGroups);
