...
//printk("Check_pattern len=%d, pagelen=%d\n");
	end = paglen + td->offs;
...
PRINTK("read primary version\n");
		this->read_raw (mtd, buf, td->pages[0] << this->page_shift, mtd->oobblock, mtd->oobsize);
...
PRINTK("read mirror version\n");
		this->read_raw (mtd, buf, md->pages[0] << this->page_shift, mtd->oobblock, mtd->oobsize);
...
static int brcmnand_create_bbt (struct mtd_info *mtd, uint8_t *buf, struct nand_bbt_descr *bd, int chip)
{
	...
	int i, j; 
	int numblocks;
	int len; /* How many pages to read per block */
	...
	for (i = startblock; i < numblocks;) {
		...
		for (j = 0; j < len; j++) {
			if (!(bd->options & NAND_BBT_SCANEMPTY)) {
				...
				ret = this->read_pageoob(mtd, __ll_add32(from,  j * mtd->oobblock), buf, &retlen, &oobsel, autoplace, raw);
				if (ret) {
					printk("%s: j=%d read_oob(from=%08x) returns %d\n", __FUNCTION__, j, __ll_low(from) + j * mtd->oobblock, ret);
					return ret;
				}

				if (check_short_pattern (buf, bd)) {
					this->bbt[i >> 3] |= 0x03 << (i & 0x6);
					printk (KERN_WARNING "Bad eraseblock 1: %d at 0x%08x\n",
						i >> 1, (unsigned int) from);
printk("read_oob returns retlen=%d, oobbuf=\n", retlen);
print_oobbuf(buf, retlen);
					break;
				}
			} else {
				if (check_pattern (&buf[j * scanlen], scanlen, mtd->oobblock, bd)) {
					this->bbt[i >> 3] |= 0x03 << (i & 0x6);
					printk (KERN_WARNING "Bad eraseblock 2: %d at 0x%08x, j=%d, scanlen=%d\n",
...
actblock, actblock << this->bbt_erase_shift, mtd->oobblock, mtd->oobsize);
			this->read_raw (mtd, buf, actblock << this->bbt_erase_shift, mtd->oobblock, mtd->oobsize);
	...
	struct nand_bbt_descr *td, struct nand_bbt_descr *md, int chipsel)
{
	...
	int i, j, res, chip = 0;
	int bits, startblock, dir, numblocks, sft, sftmsk;
	uint32_t page, offs;
...
//write_retry:
PRINTK("startblock=%08x, dir=%d, td->maxblocks=%d\n", startblock, dir, td->maxblocks);
...
PRINTK("%s: Not NAND_BBT_SAVECONTENT\n", __FUNCTION__);
		...
		if (res < 0) {
			printk (KERN_ERR "brcmnand_bbt: Error while writing bad block table %d at %08x len=%d\n", 
				res, __ll_low(to), len);
			return res;
		}
		printk (KERN_DEBUG "Bad block table written to 0x%08x, version 0x%02X\n",
			__ll_low( to ), td->version[chip]);
...
//printk("<-- %s\n", __FUNCTION__);
	...
	case 0x02:	
		return allowbbt ? 0 : 1;
	//case 0x03:	return 1; // THT added fall thru, factory/BBS/CFE marked bad
	}
	return 1;
}
