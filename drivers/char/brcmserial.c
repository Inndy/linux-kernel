...
{
#if defined(HUMAX_SUPPORT_CONSOLE)
#else
	if (offset==UART_RECV_DATA)
	{
		readb(((unsigned long) info->iomem_base + (offset<<info->iomem_reg_shift))^3);
		return 0;
	}
#endif
 ...
 */
int __init bcm3250_uart_init(void)
{
	rs_init();
}
