...
#define pmalloc(t)	kmalloc(t, GFP_ATOMIC)
#define pfree		kfree
...
extern void *pmalloc_debug(uint32_t size,char *file, int line);
extern void *pmalloc_kernel_debug(uint32_t size,char *file, int line);
#define pmalloc(s) pmalloc_debug((s),__FILE__,__LINE__)
#define pmalloc_kernel(s) pmalloc_kernel_debug((s),__FILE__,__LINE__)
#else
/*
extern void *pmalloc(uint32_t size);
extern void *pmalloc_kernel(uint32_t size);
*/
#endif
 ...
 *
 *  DESCRIPTION
 *      Obtain the size of a pmalloced memory block.
 *
 *  RETURNS
...
extern uint32_t psizeof(void *ptr);
...
extern void *zpmalloc(uint32_t size);
