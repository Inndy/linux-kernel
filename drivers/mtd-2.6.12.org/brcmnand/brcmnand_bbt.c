...
static int brcmnand_read_abs_bbt (struct mtd_info *mtd, uint8_t *buf, struct nand_bbt_descr *td, int chip)
{
	...
	if (td->options & NAND_BBT_PERCHIP) {
		...
		res = brcmnand_read_bbt (mtd, buf, td->pages[0], mtd->size >> this->bbt_erase_shift, bits, 0, td->reserved_block_code);
		if (res)
			return res;
	}
	return 0;
	...
	struct nand_bbt_descr *md)
{
	...
	if (td->options & NAND_BBT_VERSION) {
		brcmnand_read_raw (mtd, buf, td->pages[0] << this->page_shift, mtd->oobblock, mtd->oobsize);
	...
	if (md && (md->options & NAND_BBT_VERSION)) {
		brcmnand_read_raw (mtd, buf, md->pages[0] << this->page_shift, mtd->oobblock, mtd->oobsize);
...
printk("Starting for loop: bd->options=%08x\n", bd->options);
	for (i = startblock; i < numblocks;) {
		int ret;

		if (bd->options & NAND_BBT_SCANEMPTY) {
printk("Calling read_raw: from=%08x, len=%08x, ooblen=%x\n", (unsigned long) from, readlen, ooblen);
			if ((ret = brcmnand_read_raw (mtd, buf, from, readlen, ooblen))) {
				return ret;
			}
		}

//printk("brcmnand_create_bbt: read oob of entire flash, len=%x,  from=%08x, block#=%d, #blocks=%d\n", len, from, i, numblocks);
		for (j = 0; j < len; j++) {
			if (!(bd->options & NAND_BBT_SCANEMPTY)) {
				size_t retlen;
...
//else printk(".");
				...
				if (ret) {
					printk("brcmnand_create_bbt: j=%d read_oob(from=%08x) returns %d\n", j, from + j * mtd->oobblock, ret);
					return ret;
				}
...
printk("brcmnand_search_bbt: %d bbt_blocks\n", bbtblocks);
	}
	...
	for (i = 0; i < chips; i++) {
		...
		for (block = 0; block < td->maxblocks; block++) {
			...
			/* Read first page */
printk("actblock=%d, from=%08x, ooblock=%d, oobsize=%d\n", 
actblock, actblock << this->bbt_erase_shift, mtd->oobblock, mtd->oobsize);
			brcmnand_read_raw (mtd, buf, actblock << this->bbt_erase_shift, mtd->oobblock, mtd->oobsize);
...
write:
		...
		if (td->options & NAND_BBT_SAVECONTENT) {
			...
			if (res < 0) {
				if (retlen != len) {
					printk (KERN_INFO "brcmnand_bbt: Error reading block for writing the bad block table\n");
					return res;
				}
				printk (KERN_WARNING "brcmnand_bbt: ECC error while reading block for writing bad block table\n");
			}
		...
		res = this->erase_bbt (mtd, &einfo, 1);
		if (res < 0) {
			printk (KERN_WARNING "brcmnand_bbt: Error during block erase: %d\n", res);
			return res;
		}

		res = mtd->write_ecc (mtd, to, len, &retlen, buf, &buf[len], &oobinfo);
		if (res < 0) {
			printk (KERN_WARNING "brcmnand_bbt: Error while writing bad block table %d\n", res);
			return res;
		}
...
static inline int brcmnand_memory_bbt (struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	...
	return brcmnand_create_bbt (mtd, this->page_buf, bd, -1);
}
...
printk("brcmnand_scan_bbt: Allocating %d byte buffer\n", len);
	this->bbt = vmalloc (len /*, GFP_KERNEL*/);
	if (!this->bbt) {
		printk (KERN_ERR "brcmnand_scan_bbt: Out of memory, bbt_erase_shift=%d, len=%dx\n", 
...
printk("brcmnand_scan_bbt: 2 - Allocating %d byte buffer\n", len);
	buf = vmalloc (len /*, GFP_KERNEL*/);
	if (!buf) {
		printk (KERN_ERR "brcmnand_scan_bbt: Out of memory 2, bbt_erase_shift=%d, len=%dx\n", 
			this->bbt_erase_shift, len  );
		vfree (this->bbt);
		this->bbt = NULL;
		return -ENOMEM;
...
int brcmnand_default_bbt (struct mtd_info *mtd)
{
	...
	*/
	...
	//this->badblock_pattern.options &= this->options;
	return brcmnand_scan_bbt (mtd, this->badblock_pattern);
}
