...
static int bcmemac_rx_thread (void *);
#endif
...
static void tx_reclaim_timer(unsigned long arg)
{
    ...
    if ( counter++ >= HZ/TIMEOUT_10MS ) {
		...
		schedule_work(&pDevContext->link_change_task);	
...
// THT: For now we are not using this, will change to workqueue later on
		...
		pDevCtrl->counter_th++;
		pDevCtrl->cond = 1;
		atomic_inc (&pDevCtrl->nevents);
		wake_up_interruptible (&pDevCtrl->wq);
#else
...
#ifdef CONFIG_BCMINTEMAC_KTHREAD
static int bcmemac_rx_thread(void *p)
{
	struct net_device *dev = (struct net_device *)p;
 	BcmEnet_devctrl *pDevCtrl = (BcmEnet_devctrl *)dev->priv;
	atomic_t *nevents = (atomic_t *) &pDevCtrl->nevents;
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,5,0)
	daemonize ("%s", "bcmemacd");
#else
	daemonize ();
	reparent_to_init ();
#endif
	strcpy (current->comm, "bcmemacd");

	/* go into a loop and deal with events as they come */
	while (1) {
		pDevCtrl->cond = 0;
		wait_event_interruptible (pDevCtrl->wq, pDevCtrl->cond);
		if (pDevCtrl->time_to_gohome)
			complete_and_exit (&pDevCtrl->compl, 0);
...
static MII_CONFIG mii_AutoConfigure(BcmEnet_devctrl *pDevCtrl)
{
    ...
    for (i = 0; i < 1000; i++) {
#if (HUMAX_MODIFY == y) /* need scheduling... */
        msleep(1);
#else
        mdelay(1);
#endif
...
static int bcmemac_init_dev(BcmEnet_devctrl *pDevCtrl)
{
 ...
 TRACE(("bcmemacenet: bcmemac_init_dev &rxDMA=%08x, &txDma=%08x\n", 
	...
	TRACE(("bcmemac_init_dev rxFirstBd@%08x, rxLastBdPtr@%08x, RX %d\n", pDevCtrl->rxFirstBdPtr, pDevCtrl->rxLastBdPtr, i));	
...
#define GET_MACADDR_FROM_FLASH
#ifdef GET_MACADDR_FROM_FLASH //hee.temp
#define ENV_ADDR	0xBFC00400
static uint8 asc2int(uint8 msb, uint8 lsb, uint8 *res)
{
	uint8 digit,digit2;
...
static uint8 check_macaddr_range(char val)
{
...
static uint8 cnv_str2hex(char val)
{
		if( val >= 0x30 && val <= 0x39 )
		{
			return (val-0x30);
		...
		if( val >= 0x41 && val <= 0x46 )
		{
			return (val-0x41+0x0A);
		}
		if( val >= 0x61 && val <= 0x66 )
		{
			return (val-0x61+0x0A);
		}
		return 0; 
}

static uint8 parse_macaddr_cmdline(char *cmdline, char *macaddr)
{
	char *start_str = NULL;
	int fixed_offset = 7;
	int i;
	
	start_str = strstr(cmdline,"macaddr=");

	if(start_str == NULL)
	{
		return 1;
	}
	if (start_str && start_str != cmdline && *(start_str - 1) != ' ')
	{
		start_str = strstr(start_str, " macaddr=");
		if(start_str == NULL)
		{
			return 1;
		}		
	...
	if( (start_str - cmdline) > (COMMAND_LINE_SIZE - CMDLINE_MACSTR_SIZE))
	{
		return 1;
	}	
	for(i=0;i<18;i++)
	{
		...
		{
			if(check_macaddr_range(start_str[fixed_offset+i]) == 1)
				return 1;
		}
	}
	for(i=0;i<6;i++)
	{
		macaddr[i] = cnv_str2hex(start_str[fixed_offset+1+(i*3)])*0x10+cnv_str2hex(start_str[fixed_offset+1+(i*3)+1]);	
	}
	return 0;		
}

static uint8 bcmemac_getMacAddrtemp(uint8 *flash_eaddr)
{
#if 0 //hee.new
	void *virtAddr;
	int i,j;
	uint8 *tempAddr;
	virtAddr = (void *)ENV_ADDR; 
	tempAddr=(uint8 *)virtAddr;
	for(i=0;i<0x1FF00;i++)
	{
		if(*(tempAddr+i)==0x68 &&	*(tempAddr+i+1)==0x77 &&	*(tempAddr+i+2)==0x61 &&	*(tempAddr+i+3)==0x64 \
		   &&	*(tempAddr+i+4)==0x64 &&	*(tempAddr+i+5)==0x72 &&	*(tempAddr+i+6)==0x3D)
		{
			for(j=0;j<6;j++)
				if(asc2int(*(tempAddr+i+7+(j*3)),*(tempAddr+i+7+(j*3+1)),&flash_eaddr[j]))
					return 1;
		
			break;
		}
	}
	if(i==0x1FF00) return 1;
	
	return 0;
#else
	char temp_command_line[COMMAND_LINE_SIZE];
	char temp_macaddr[6];
	uint8 *virtAddr;
	int i;
	
	strlcpy(temp_command_line, saved_command_line, COMMAND_LINE_SIZE);
	
	memset(temp_macaddr,0xFF,6);
	if(parse_macaddr_cmdline(temp_command_line, temp_macaddr) == 0)
	{
		virtAddr = (uint8 *)temp_macaddr;
	}
	else
	{
		printk("No Mac Address Specified\n");	
		virtAddr = (uint8 *)ENV_ADDR; 		
	}
	

	for(i=0;i<6;i++)
	{
		flash_eaddr[i] = virtAddr[i];	
	}
	
	return 0;
#endif
}
#endif //GET_MACADDR_FROM_FLASH(hee.temp)

static void bcmemac_getMacAddr(struct net_device* dev)
{
	uint8 flash_eaddr[ETH_ALEN]={0x00,0x03,0x78,0x09,0x74,0x01};
#ifdef GET_MACADDR_FROM_FLASH //hee.temp
	uint8 flash_emac[ETH_ALEN];
#endif //GET_MACADDR_FROM_FLASH(hee.temp)
	void *virtAddr;
	uint16 word;
	int i;
	 ...
	 */
	printk("Getting MAC address...\n");

#ifdef GET_MACADDR_FROM_FLASH //hee.temp
	if(bcmemac_getMacAddrtemp(flash_emac) == 0x00) //MAC Addr found!
	{
		memcpy(flash_eaddr,flash_emac,6);		
	}
#endif //GET_MACADDR_FROM_FLASH(hee.temp)
#if 0
	word=0;
...
#ifdef CONFIG_BCMINTEMAC_KTHREAD
	pDevContext->cond = 0;
	pDevContext->time_to_gohome = 0;
	pDevContext->pid = 0;
	init_waitqueue_head(&pDevContext->wq);
	init_completion(&pDevContext->compl);
	pDevContext->counter_bh = 0;
	pDevContext->counter_th = 0;
	atomic_set (&pDevContext->nevents, 0);
	pDevContext->catchup = 0;
#endif
	...
	if (probed == 0) {
		...
		if (dev == NULL || pDevContext == NULL) {
			printk(KERN_ERR CARDNAME": Unable to allocate "
				"net_device structure! (%d bytes)\n", sizeof(struct BcmEnet_devctrl) + sizeof(struct net_device));
			return -ENODEV;
		}
	}
...
#ifdef CONFIG_BCMINTEMAC_KTHREAD
    if ((pDevContext->pid = kernel_thread (bcmemac_rx_thread, (void *) dev,
							   CLONE_FS | CLONE_FILES)) < 0) {
		 printk (KERN_ERR CARDNAME": Failed to generate a kernel thread\n");
 		 return -ENOMEM;
	 }
#endif

#ifdef CONFIG_BCMINTEMAC_NETLINK
	INIT_WORK(&pDevContext->link_change_task, (void (*)(void *))bcmemac_link_change_task, pDevContext);
	printk("init link_change_task");
#endif			

	TRACE(("<-- bcmemac_net_probe\n"));
	/* device has already been initialized */
#if (HUMAX_MODIFY == y) /* 정상 setting이 끝난 후임.. */
	return 0;
#else
	return -ENXIO;
#endif
...
static void bcmemac_init_cleanup(struct net_device *dev)
{
   ...
   BcmEnet_devctrl *pDevCtrl = pDevContext;
	...
	{
		pDevCtrl->cond = 1;
		pDevCtrl->time_to_gohome = 1;
		wake_up_interruptible (&pDevCtrl->wq);
		wait_for_completion (&pDevCtrl->compl);
	}
#endif
