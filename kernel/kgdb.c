...
static void put_packet(char *buffer)
{
	...
	while (1) {
		...
		if (ch == '$') {
			...
			PRINTK("%s gdb reconnecting. done.\n",__FUNCTION__);
			return;
		}
	}
}
...
static int kgdb_remove_sw_break(unsigned long addr)
{
	...
	for (i = 0; i < MAX_BREAKPOINTS; i++) {
		if ((kgdb_break[i].state == bp_enabled) &&
            ...
            /* Mark the breakpoint as disabled now incase a memory
             * operation fails from a fault.
             */
            ...
            {
                /* Test a read before a write to make sure the page still
                 * exists.
                 */
                if ((error = kgdb_get_mem((char *)addr,
                                          saved_instr,
            ...
            kgdb_may_fault = 0;
			PRINTK("removed bp at 0x%x\n",(int) addr);
			return 0;
		}
	}

	PRINTK("%s ERROR: no bp at 0x%x\n",__FUNCTION__,(int) addr);
...
int remove_all_break(void)
{
	...
	for (i = 0; i < MAX_BREAKPOINTS; i++) {
        /* We want to make a valiant attempt to remove every single
         * breakpoint so we have to pay the price of a memory probe
         * each time.
         */
        kgdb_may_fault = 1;
        if ((kgdb_fault_setjmp(kgdb_fault_jmp_regs)) != 0) {
			  ...
			  struct pt_regs *linux_regs)
{
	...
	if (atomic_read(&debugger_active) == smp_processor_id() + 1)
		{
		panic ("kgdb_handle_exception: recursive enter\n");
		return 0;
		}
...
void kgdb_unregister_io_module(struct kgdb_io *local_kgdb_io_ops)
{
	...
	if (kgdb_io_handler_cnt > 0 && kgdb_io_ops_prev[0].init != NULL) {
		/* First check if the hook that is in use is the one being removed */
		if (kgdb_io_ops.init == local_kgdb_io_ops->init) {
			/* Set 'i' to the value of where the list should be shifed */
			i = kgdb_io_handler_cnt - 1;
			memcpy(&kgdb_io_ops, &kgdb_io_ops_prev[i],
			       sizeof(struct kgdb_io));
		} else {
			/* Simple case to remove an entry for an I/O handler that is not in use */
		...
		/* Handle the case if we are on the last element and set it to NULL; */
