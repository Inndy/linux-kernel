	 	...
	 	list_for_each(lh, &dev->objectBucket[i].list) {
			if (lh) {
				...
				if (!obj->deferedFree) {
					...
					//if (cp.chunkId >= 0)  // THT Do not write out unused blocks. (chunkId=-1)
					{
						ok = (yaffs_CheckpointWrite(dev,&cp,sizeof(cp)) == sizeof(cp));
...
//printk("%s: Scanning block %d, chunk %d\n", __FUNCTION__, blk, chunk);
			result = yaffs_ReadChunkWithTagsFromNAND(dev, chunk, data,
							&tags);
...
#if 0
			/*THT Take care of erased blocks */
			else if (!dev->isYaffs2 &&tags.chunkId < 0) {
				if (yaffs_CheckFF(data, dev->nDataBytesPerChunk) || tags.chunkUsed) {
